    ...
    >
      ...
      <motion.div
        ...
        className={cn(
          ...
          "border-gray-200 bg-background shadow-2xl will-change-transform",
  ...
  function AnimatedSidebar(
    ...
    return (
      <motion.aside
        ...
        style={style}
        className={cn(
          "peer group/sidebar relative hidden h-auto shrink-0 md:block will-change-[width]",
...
>(function AnimatedSidebarInset({ className, ...props }, forwardedRef) {
  return (
    <motion.main
      ...
      className={cn(
        "relative z-10 flex h-screen min-w-0 flex-1 flex-col bg-white max-h-screen",
        className,
      )}
    />
  );
...
>(function AnimatedSidebarGroupLabel(
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mb-1 h-7 overflow-hidden px-2 text-[13px] font-medium lowercase tracking-[0.04em] text-muted-foreground",
        className,
      )}
    >
      {children}
    ...
    <AnimatePresence initial={false} mode="popLayout">
      {open && !panel.collapsed ? (
        <motion.ul
          ...
          className={cn(
            "relative mt-1 ml-5 flex min-w-0 flex-col gap-0.5 border-gray-200 border-l pl-3",
            className,
          )}
        >
          {children}
